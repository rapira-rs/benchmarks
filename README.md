...
Rapira in its worker, classic, and dispatcher modes against other PHP application servers, on a set of PHP apps from a hello page and static files to a gRPC echo service. `suites/targets.toml` lists every target, and the `ci` suite runs 14 of them after each nightly build of rapira main.
