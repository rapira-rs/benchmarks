"""Tests of the box scripts: start, probe, and stop the rapira server, and the byte-exact probe.

The lifecycle tests use /opt/bench and the port 8080, so they run only in the
...
# The stub of "rapira serve CONFIG". A copy of python3 named rapira runs this file from its
# working directory, $BENCH/run, so /proc/<pid>/exe is the copy.
STUB = r'''
import os
import signal
import socketserver
...
class BoxLifecycleTests(unittest.TestCase):
    ...
    def reset(self):
        ...
        for directory in ("box", "servers", "apps/hello", "apps/grpc"):
            shutil.copytree(REPO / directory, RIG / directory)
        for directory in ("run", "log", "rapira"):
    ...
    def force_cleanup(self):
        subprocess.run(["pkill", "-KILL", "-u", str(os.getuid()), "-f", "opt/bench"], check=False)
        deadline = time.monotonic() + 3
        while time.monotonic() < deadline and self.leftovers():
            time.sleep(0.05)

    def leftovers(self):
        return subprocess.run(["pgrep", "-u", str(os.getuid()), "-f", "opt/bench"], capture_output=True, text=True).stdout.split()
    ...
    def test_lifecycle(self):
        for case in LIFECYCLE_CASES:
            with self.subTest(name=case["name"]):
                ...
                inspected = self.target("probe", TAG, "rapira")
                ...
                memory = self.target("mem", TAG, "rapira")
                self.assert_success(memory)
                self.assertGreater(int(memory.stdout), 0)
                self.assert_success(self.target("stop", TAG, "rapira"))
            ...
            with self.subTest(name=case["name"]):
                ...
                if case["hold_before"]:
                    with hold(case["hold_before"]):
                        result = self.target(*self.start_arguments(case), env=case["env"])
                else:
                    result = self.target(*self.start_arguments(case), env=case["env"])
