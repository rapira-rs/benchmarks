<?php
// Symfony worker for FrankenPHP: the kernel boots once per worker, the
// handler runs per request. Byte-identical to worker-rapira.php except the
// loop primitive; both mirror runtime/frankenphp-symfony's Runner (env
// snapshot merge, send(), terminate outside the handler, gc per cycle), so
// the pair compares servers, not bridge code. Installed into public/ so
// php_server resolves requests to the worker script.
